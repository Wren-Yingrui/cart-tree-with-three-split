...
class EditCartTree_three_split():
    def __init__(self,
                 min_samples_split,
                 min_samples_leaf,
                 min_gini,
    ...
    def split(self,x_series, y, min_samples_split):
        import pandas as pd
        import numpy as np
        import math
                ...
                if i < min_samples_split:
                    ...
                    for j in range(i, len(x_series)):
                        if len(x_series) - j < min_samples_split:
                            ...
                            gini1 = 1 - sum(pow(dataframe_tmp[:i + 1].groupby('y').count() / (i + 1), 2))[0]
                            gini2 = np.NaN
                            gini3 = 1 - \
                                    sum(pow(dataframe_tmp[i + 1:].groupby('y').count() / (len(x_series) - i - 1), 2))[0]
                            split_point_score[str(i) + '_' + str(j)] = (i + 1) / len(x_series) * gini1 + (
                                        len(x_series) - i - 1) / len(x_series) * gini3
                            gini_group[str(dataframe_tmp.loc[i, 'x']) + '_' + str(dataframe_tmp.loc[j, 'x'])] = [gini1,
                                                                                                                 gini2,
                                                                                                                 gini3]
                        else:
                            ...
                            gini3 = 1 - pow(dataframe_tmp[j + 1:].groupby('y').count() / (len(x_series) - j - 1),
                                            2).sum()[0]
                            split_point_score[str(dataframe_tmp.loc[i, 'x']) + '_' + str(dataframe_tmp.loc[j, 'x'])] = (
                                                                                                                                   i + 1) / len(
                                x_series) * gini1 + \
                                                                                                                       (
                                                                                                                                   j - i) / len(
    ...
    def choose_best_feature(self,x, y, min_samples_split):
        import numpy as np
        gini_list = {}
        split_1_list = {}
        split_2_list = {}
        ...
        for x_col in x.columns:
            split_1, split_2, gini, [gini1, gini2, gini3] = self.split(x[x_col], y, min_samples_split)
            if split_1 != np.NaN:
    ...
    def create_decision_tree_first_split(self,x, y, min_samples_split, min_gini, min_samples_leaf,num):
        import pandas as pd
        import numpy as np
        import math
        best_feature, best_split_1, best_split_2, gini, [gini1, gini2, gini3] \
            = self.choose_best_feature(x, y, min_samples_split)
        tree = {best_feature: {}}
    ...
    def create_decision_tree_split_twice(self,x, y, min_samples_split, min_gini, min_samples_leaf, tree):
        import pandas as pd
        import numpy as np
        print(x)
        dataframe_tmp = pd.DataFrame(x)
            ...
            for key_1, value_1 in value.items():
                if value_1 == {}:
                    if '<=' in key_1:
                        ...
                        subtree = self.create_decision_tree_first_split(x1, y1, min_samples_split, min_gini,
                        ...
                        for key_3, value_3 in value_2.items():
                            for key_4, value_4 in value_3.items():
                                if '<=' in key_4 and x_predict[key_3].item() <= float(key_4.replace('<=', '')):
                                    if value_4 in [0, 1]:
                                        ...
                                        for key_5, value_5 in value_4.items():
                                            for key_6, value_6 in value_5.items():
                                                if '<=' in key_6 and x_predict[key_5].item() <= float(
                                                        key_6.replace('<=', '')):
                                                    y = value_6
                                                    print(3)
                                                    break
                                                elif '-' in key_6 and x_predict[key_5].item() > float(
                                                        key_6.split('-')[0]) and x_predict[key_5].item() <= float(
                                                        key_6.split('-')[1]):
                                                    y = value_6
                                                    print(4)
                                                    break
                                                    ...
                                                    pass
                                elif '-' in key_4 and x_predict[key_3].item() > float(key_4.split('-')[0]) and \
                                        x_predict[key_3].item() <= float(key_4.split('-')[1]):
                                    if value_4 in [0, 1]:
                                        y = value_4
                                        print(6)
                                        break
                                    ...
                                    pass
                elif '-' in key_2 and x_predict[key_1].item() > float(key_2.split('-')[0]) and x_predict[
                    key_1].item() <= float(
                        key_2.split('-')[1]):
                    if value_2 in [0, 1]:
                        y = value_2
    ...
    def plot_auc(self,y_predict, y_test):
    ...
    def confussion_matrix(self,y_predict, y_test):
        from sklearn.metrics import confusion_matrix
        return confusion_matrix(y_test, y_predict)
    def classification_report(self,y_predict, y_test):
        from sklearn.metrics import classification_report
        report = classification_report(y_test, y_predict, output_dict=True)
        return report
if __name__ == '__main__':
    data=pd.read_csv('C:/Users/任英睿/PycharmProjects/pythonProject/Financial_risk/TrainData.csv',encoding='gbk')[:200]
    x=data[['RevolvingUtilizationOfUnsecuredLines','age','DebtRatio','MonthlyIncome']]
    y=data['SeriousDlqin2yrs']
    three_decision_tree=EditCartTree_three_split(2,
                 2,
                 0.2,
                 x,
                 y)
    tree=three_decision_tree.create_decision_tree_first_split(x, y, 2, 0.1, 2)
    tree=three_decision_tree.create_decision_tree_split_twice(x, y, 2, 0.1, 2, tree)
    x_predict = \
    pd.read_csv('C:/Users/任英睿/PycharmProjects/pythonProject/Financial_risk/TrainData.csv', encoding='gbk')[2000:2100][
        ['RevolvingUtilizationOfUnsecuredLines', 'age', 'DebtRatio', 'MonthlyIncome']]
    y_predict = x_predict.apply(lambda x: three_decision_tree.predict(x, tree), axis=1)
    y_true = \
    pd.read_csv('C:/Users/任英睿/PycharmProjects/pythonProject/Financial_risk/TrainData.csv', encoding='gbk')[2000:2100][
        'SeriousDlqin2yrs']
    three_decision_tree.plot_auc(y_predict, y_true)
    data_confusion = three_decision_tree.confussion_matrix(y_predict,y_true)
    pd.DataFrame(data_confusion).to_excel('C:/Users/任英睿/PycharmProjects/pythonProject/cart树改进/data_confusion.xlsx',
                                          index=False)
    report=three_decision_tree.classification_report(y_predict,y_true)
    pd.DataFrame(report).to_excel('C:/Users/任英睿/PycharmProjects/pythonProject/cart树改进/report.xlsx',
                                          index=False)
