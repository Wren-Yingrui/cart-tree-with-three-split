...
def split(x_series,y,min_samples_split):
    ...
    if len(x_series)<min_samples_split:
        ...
        for i in range(0,len(x_series)):
            if i<min_samples_split:
                ...
                for j in range(i,len(x_series)):
                    if 0<j-i < min_samples_split:
                        pass
                    else:
                        for k in range(j,len(x_series)):
                            if len(x_series)-k<min_samples_split or 0<k-j < min_samples_split:
                                pass
                            elif i==j and j==k:
                                gini1=1-pow(dataframe_tmp[:i+1].groupby('y').count()/(i+1),2).sum()[0]
                                gini2=np.NaN
                                gini3 = np.NaN
                                gini4=1-pow(dataframe_tmp[i+1:].groupby('y').count()/(len(x_series)-i-1),2).sum()[0]
                                split_point_score[str(dataframe_tmp.loc[i,'x'])+'_'+str(dataframe_tmp.loc[j,'x'])+'_'+str(dataframe_tmp.loc[k,'x'])]=(i+1)/len(x_series)*gini1+(len(x_series)-i-1)/len(x_series)*gini4
                                gini_group[str(dataframe_tmp.loc[i,'x'])+'_'+str(dataframe_tmp.loc[j,'x'])+'_'+str(dataframe_tmp.loc[k,'x'])]=[gini1,gini2,gini3,gini4]
                            elif i==j and k-j>=min_samples_split:
                                gini1=1-pow(dataframe_tmp[:i+1].groupby('y').count()/(i+1),2).sum()[0]
                                gini2=np.NaN
                                gini3 = 1 -pow(dataframe_tmp[i+1:k+1].groupby('y').count() / (k-j), 2).sum()[0]
                                gini4 = 1 - pow(dataframe_tmp[k+1:].groupby('y').count() / (len(x_series) - j - 1),
                                        2).sum()[0]
                                split_point_score[str(dataframe_tmp.loc[i,'x'])+'_'+str(dataframe_tmp.loc[j,'x'])+'_'+str(dataframe_tmp.loc[k,'x'])] \
                                    = (i + 1) / len(x_series) * gini1 + (k - j)/len(x_series) * gini3 + \
                                        (len(x_series) - k - 1) / len(x_series) * gini4
                                gini_group[str(dataframe_tmp.loc[i,'x'])+'_'+str(dataframe_tmp.loc[j,'x'])+'_'+str(dataframe_tmp.loc[k,'x'])] = [gini1,gini2,gini3,gini4]
                            elif k==j and j-i>=min_samples_split:
                                gini1 = 1 -pow(dataframe_tmp[:i+1].groupby('y').count() / (i + 1), 2).sum()[0]
                                ...
                                gini4 = 1 - pow(dataframe_tmp[k+1:].groupby('y').count() / (len(x_series) - j - 1),
                                        2).sum()[0]
                                split_point_score[str(dataframe_tmp.loc[i,'x'])+'_'+str(dataframe_tmp.loc[j,'x'])+'_'+str(dataframe_tmp.loc[k,'x'])] \
                                    = (i + 1) / len(x_series) * gini1 + (j - i)/len(x_series) * gini2 + \
                                        (len(x_series) - k - 1) / len(x_series) * gini4
                                gini_group[str(dataframe_tmp.loc[i,'x'])+'_'+str(dataframe_tmp.loc[j,'x'])+'_'+str(dataframe_tmp.loc[k,'x'])] = [gini1,gini2,gini3,gini4]
                            elif k!=j and j-i>=min_samples_split and k-j>=min_samples_split:
                                gini1 = 1 -pow(dataframe_tmp[:i+1].groupby('y').count() / (i + 1), 2).sum()[0]
                                gini2 = 1 -pow(dataframe_tmp[i+1:j+1].groupby('y').count() / (j-i), 2).sum()[0]
                                gini3 = 1 -pow(dataframe_tmp[j+1:k+1].groupby('y').count() / (k-j), 2).sum()[0]
                                gini4 = 1 - pow(dataframe_tmp[k+1:].groupby('y').count() / (len(x_series) - j - 1),
                                        2).sum()[0]
                                split_point_score[str(dataframe_tmp.loc[i,'x'])+'_'+str(dataframe_tmp.loc[j,'x'])+'_'+str(dataframe_tmp.loc[k,'x'])] \
                                    = (i + 1) / len(x_series) * gini1 + (j - i)/len(x_series) * gini2 + \
        ...
        try:
            ...
            [gini1,gini2,gini3,gini4]=gini_group[split_point]
            return split_point.split('_')[0],split_point.split('_')[1],split_point.split('_')[2],gini,[gini1,gini2,gini3,gini4]
        except:
...
def choose_best_feature(x, y, min_samples_split):
    ...
    for x_col in x.columns:
        split_1, split_2, split_3,gini,[gini1,gini2,gini3,gini4] = split(x[x_col], y, min_samples_split)
        if split_1!=np.NaN:
            gini_list[x_col] = gini
            split_1_list[x_col] = split_1
            split_2_list[x_col] = split_2
            split_3_list[x_col] = split_3
            gini_group_list[x_col] = [gini1,gini2,gini3,gini4]
    if len(gini_list.keys())>0:
        best_feature = min(gini_list.keys(), key=(lambda x: gini_list[x]))
        return best_feature,\
               split_1_list[best_feature],\
           split_2_list[best_feature], \
            split_3_list[best_feature], \
...
def create_decision_tree_first_split(x, y, min_samples_split, min_gini, min_samples_leaf, num):
    ...
    import numpy as np
    import math
    best_feature, best_split_1, best_split_2, best_split_3,gini, [gini1, gini2, gini3,gini4] \
    ...
    #     return tree
        ...
        best_split_3 = float(best_split_3)
        print('分裂的最优列:', best_feature)
        print(best_split_1, best_split_2,best_split_3)
        dataframe_tmp = pd.DataFrame(x)
        dataframe_tmp['y'] = y
        subtree = {}
        if str(gini2) == 'nan' and str(gini3) == 'nan':
            ...
            if gini4 < min_gini or num == 3 or len(
                    dataframe_tmp[dataframe_tmp[best_feature] > best_split_3]) <= 2 * min_samples_leaf:
                y4 = dataframe_tmp[dataframe_tmp[best_feature] > best_split_3].groupby('y')[best_feature].count() \
                    [dataframe_tmp[dataframe_tmp[best_feature] > best_split_3].groupby('y')[best_feature].count() ==
                     max(dataframe_tmp[dataframe_tmp[best_feature] > best_split_3].groupby('y')[
                             best_feature].count())].index[0]
                subtree['>=' + str(best_split_3)] = y4
            else:
                subtree['>=' + str(best_split_3)] = {}
        elif str(gini2) == 'nan' and str(gini3) != 'nan':
            if gini1 < min_gini or num == 3 or len(
                    dataframe_tmp[dataframe_tmp[best_feature] <= best_split_1]) <= 2 * min_samples_leaf:
                ...
                subtree['<=' + str(best_split_1)] = {}
            if gini3 < min_gini or num == 3 or len(dataframe_tmp[(dataframe_tmp[best_feature] > best_split_2) & (
                    dataframe_tmp[best_feature] <= best_split_3)]) <= 2 * min_samples_leaf:
                y3 = dataframe_tmp[(dataframe_tmp[best_feature] > best_split_2) & (
                        dataframe_tmp[best_feature] <= best_split_3)].groupby('y')[best_feature].count() \
                    [dataframe_tmp[(dataframe_tmp[best_feature] > best_split_2) & (
                            dataframe_tmp[best_feature] <= best_split_3)].groupby('y')[best_feature].count() ==
                     max(dataframe_tmp[(dataframe_tmp[best_feature] > best_split_2) & (
                             dataframe_tmp[best_feature] <= best_split_3)].groupby('y')[
                             best_feature].count())].index[0]
                subtree[str(best_split_2) + '-' + str(best_split_3)] = y3
            else:
                subtree[str(best_split_2) + '-' + str(best_split_3)] = {}
            if gini4 < min_gini or num == 3 or len(
                    dataframe_tmp[dataframe_tmp[best_feature] > best_split_3]) <= 2 * min_samples_leaf:
                y4 = dataframe_tmp[dataframe_tmp[best_feature] > best_split_3].groupby('y')[best_feature].count() \
                    [dataframe_tmp[dataframe_tmp[best_feature] > best_split_3].groupby('y')[best_feature].count() ==
...
x_predict=pd.read_csv('C:/Users/任英睿/PycharmProjects/pythonProject/Financial_risk/TrainData.csv',encoding='gbk')[3300:3340][['RevolvingUtilizationOfUnsecuredLines','age','DebtRatio','MonthlyIncome']]
y_predict=x_predict.apply(lambda x:predict(x,tree),axis=1)
y_true=pd.read_csv('C:/Users/任英睿/PycharmProjects/pythonProject/Financial_risk/TrainData.csv',encoding='gbk')[3300:3340]['SeriousDlqin2yrs']
plot_auc(y_predict,y_true)
data_confusion=confussion_matrix(y_predict,y_true)
print(data_confusion)
pd.DataFrame(data_confusion).to_excel('C:/Users/任英睿/PycharmProjects/pythonProject/cart树改进/data_confusion_4.xlsx',index=False)
report=classification_report(y_predict,y_true)
print(report)
pd.DataFrame(report).to_excel('C:/Users/任英睿/PycharmProjects/pythonProject/cart树改进/report_4.xlsx',index=False)
